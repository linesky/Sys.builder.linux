...
sudo dd if=/dev/zero of="/tmp/my.img" bs=1k count=180000
#sudo dd if=/dev/zero of="/tmp/image" bs=1k count=5000
cp /initrd.img "/tmp/image"
...
cp /tmp/prints $roots/usr/bin
cp /tmp/prints $roots/bin

cp /initrd.img $roots/boot
...
cp  /usr/lib/syslinux/modules/bios/* $roots/boot/syslinux
cp  /usr/lib/syslinux/modules/* $roots/boot/syslinux
